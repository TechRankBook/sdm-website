...
serve(async (req)=>{
  ...
  try {
    ...
    const userInfo = await supabase.from('users').select('id, email, phone_no').eq('id', user.id).single();
    ...
    const orderData = {
      ...
      currency: 'INR',
      receipt: `rcpt_${bookingId.slice(-32)}`,
      payment_capture: 1,
      notes: {
    ...
    logStep('Razorpay order created', {
      orderId: order.id,
      amount: order.amount
    });
    ...
    await supabaseServiceRole.from('bookings').update({
    ...
    return new Response(JSON.stringify({
      ...
      user_name: userInfo.data?.full_name || userInfo.data || 'Guest',
      user_phone: userInfo.data?.phone_no || user.user_metadata?.phone_number || 'Not provided'
    }), {
      headers: {
        ...corsHeaders,
        'Content-Type': 'application/json'
    ...
    logStep('ERROR in create-razorpay-order', {
